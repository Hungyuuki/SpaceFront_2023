...
const getAllNamesOf = (className: string = '.room-name'): string => {
  const names: string[] = [];
  ...
  for (const item of users) {
    ...
    if (index != -1) {
      const user = {
        ...
        uid: item.uid,
        user_login_status: item.user_login_status,//số index của status, hiểu sai sẽ dễ truyền sai
...
const readyToChatOnLoad = (user: any): string => {
...
const convertUnicodeToString = (unicode: string): string => {
  if (unicode.split(',').length > 1) {
    return unicode.split(',')
      .map((item: any) => String.fromCodePoint(item))
      .join("");
  }
  return unicode;
}
...
function createFLoorsHTML(floors: any, floor_id: any, role: any) {
  ...
  for (let i = 0; i < floors.length; i++) {
    ...
    floorsHTML += createFLoorElement(floors[i],
      floors[i].id == floor_id ? 'rgb(252,76,86)': 'rgb(238, 238, 238)',      //Set màu nền, click thì đỏ, ko click thì xám
...
const renderUserHTML = (user: any): string => {
  let loginStatus = statusUser[user.user_login_status] ?? '';
  if (user.login_status == CUSTOM_STATUS) {
    loginStatus = user.custom_status
  }
  const colorBackroundStatus = colorStatus[user.login_status] ?? '';
...
const onChangeMicEvent = (user: any) => {
  ...
  if (onChannel(user.room_id) && !offStatus(user.userId)) {
    ...
    userElement?.classList.remove('unactive');
  }
  if (!onChannel(user.room_id)) {
    resetActiveUsersInRoom(user.room_id);
  }
...
const offStatus = (userId : string | number) => {
  const micOff = document.querySelector(`#mic-off-${userId}`) as HTMLInputElement;
  const speakerOff = document.querySelector(`#speaker-off-${userId}`) as HTMLInputElement;
...
const waitingConnectAgora = (userId: string | number) => {
  const userElement = document.getElementById(`user-${userId}`);
  userElement?.classList.remove('active');
  userElement?.classList.add('unactive');
}

const toggleStatus = (roomId: string | number, from: string, to: string): void => {
  const roomElement = document.querySelectorAll(`#room-${roomId} .user`) as NodeListOf<HTMLElement>;
...
const changeActiveUsersInRoom = (roomId: string | number, users : any[] = []): void => {
  const roomElement = document.querySelectorAll(`#room-${roomId} .user`) as NodeListOf<HTMLElement>;
  for (let i = 0; i < roomElement.length; i++) {
    const id = roomElement[i].getAttribute('id');   
    const userId = id.substring(id.lastIndexOf('-') + 1); 
...
const resetActiveUsersInRoom = (roomId: string | number): void => {
  const roomElement = document.querySelectorAll(`#room-${roomId} .user`) as NodeListOf<HTMLElement>;
  for (let i = 0; i < roomElement.length; i++) {
    const id = roomElement[i].getAttribute('id');
    const userId = id.substring(id.lastIndexOf('-') + 1);
...
const onReadyToChatEvent = (data: any) => {
  const roomElement = document.querySelectorAll(`#room-${data.roomId} .user`) as NodeListOf<HTMLElement>;
  for (let i = 0; i < roomElement.length; i++) {
    const id = roomElement[i].getAttribute('id');
    const userId = id.substring(id.lastIndexOf('-') + 1);
    if(isValid(userId)) {
...
const onDisConnectNetWorkEvent = (userId: any) => {
  const user = document.getElementById(`user-${userId}`) as HTMLElement;
  if (user != null) {
...
const onUserConnectedEvent = (userId: any) => {
  const user = document.getElementById(`user-${userId}`) as HTMLElement;
  if (user != null) {
    const id = user.parentElement.getAttribute('id');
    if (onChannel(id.substring(id.lastIndexOf('-') + 1))) {
...
const renderMembersHTML = (members: any) => {
  return members.map((member : any) => renderMemberHTML(member)).join('');
}
...
const toggleActiveStatus = (from: string, to: string) => {
  const footer = document.querySelector(`.footer`);
  footer?.classList.remove(from);
  footer?.classList.add(to);
}

const onChannel = (roomId: number | string): boolean => {
...
const isValid = (userId: string | number) => {
  const statusMic = document.querySelector(`#user-${userId} .mic .fa-microphone`) as HTMLElement;
  const statusSpeaker = document.querySelector(`#user-${userId} .headphone i`) as HTMLElement;
...
const getUsersMicOnInRoom = (roomId : number | string) : string[] => {
  const usersElement = document.querySelectorAll(`#room-${roomId} .user .mic .fa-microphone`) as NodeListOf<HTMLElement>;
  const userIdsMicOn = [];
...
const getUsersSpeakerOnInRoom = (roomId : number | string) : string[] => {
  const usersElement = document.querySelectorAll(`#room-${roomId} .user .headphone i`) as NodeListOf<HTMLElement>;
...
const toggleActiveStatusByFriendSide = (userId: number | string, from: string, to: string) => {
  const user = document.getElementById(`user-${userId}`);
  user?.classList.remove(from);
...
const resetStatus = () => {
  ...
  footer?.classList.remove('unactive');
}

const resetStatusByFriendSide = (roomId: string | number): void => {
  const roomElement = document.querySelectorAll(`#room-${roomId} .user`) as NodeListOf<HTMLElement>;
  for (let i = 0; i < roomElement.length; i++) {
    const id = roomElement[i].getAttribute('id');
    if (roomId == localStorage.getItem('roomId')) {
...
setInterval(() => {
  ...
  onEventHandler(SOCKET_EVENT.CONNECTED, async (data: any) => {
    if (data.isJoinRoom && onChannel(localStorage.getItem('roomId')) && readyToConnectAgora()) {
      toggleActiveStatus('unactive', 'active');
    }
    await window.api.send('close-modal')
    load();
  })
  onEventHandler(SOCKET_EVENT.DISCONNECTING, (data : any) => {
